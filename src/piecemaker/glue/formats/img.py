...
class ImageFormat(BaseFormat):
    ...
    def output_filename(self, *args, **kwargs):
        ...
        if (
            self.sprite.config["css_cachebuster_filename"]
    ...
    def _raw_canvas(self):
        ...
        for image in self.sprite.images:
            canvas.paste(
                ...
                (
                    round_up(
                        image.x
                        + (image.padding[3] + image.margin[3]) * self.sprite.max_ratio
                    ),
                    round_up(
                        image.y
                        + (image.padding[0] + image.margin[0]) * self.sprite.max_ratio
                    ),
                ),
            )
        ...
        if self.sprite.config["png8"]:
            ...
            canvas = canvas.convert("RGB").convert(
