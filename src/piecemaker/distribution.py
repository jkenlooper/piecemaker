...
def random_pos_outside_of_outline(table_bbox, outline_bbox, bbox):
    ...
    left_side_area = bbox_area(left_side_bbox)
    top_middle_bbox = [
    ...
    top_middle_area = bbox_area(top_middle_bbox)
    bottom_middle_bbox = [
        outline_bbox[0] - piece_width,
        outline_bbox[3],
    ...
    bottom_middle_area = bbox_area(bottom_middle_bbox)
    right_side_bbox = [
        outline_bbox[2],
        table_bbox[1],
    ...
    right_side_area = bbox_area(right_side_bbox)
    box = choices(
        [left_side_bbox,
         top_middle_bbox,
         bottom_middle_bbox,
...
def random_outside(table_bbox, outline_bbox, piece_bboxes):
    rtree_idx = index.Index(interleaved=True)
    outline_id = len(piece_bboxes) + 1
    rtree_idx.insert(outline_id, outline_bbox)
    positions = {}

    def nonoverlapping(bbox, attempt=15):
        (x, y) = random_pos_outside_of_outline(table_bbox, outline_bbox, bbox)
        if attempt > 0:
            count = rtree_idx.count([x, y, x + (bbox[2] - bbox[0]), y + (bbox[3] - bbox[1])])
            if count > 0:
                return nonoverlapping(bbox, attempt=attempt - 1)
            else:
                return [x, y]
        else:
            return [x, y]
